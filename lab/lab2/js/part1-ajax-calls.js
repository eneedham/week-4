...
_.each(theArray, function(arr){
  console.log(logDouble(arr));
});
/* =====================
...
var solarData = $.ajax("https://raw.githubusercontent.com/CPLN690-MUSA610/datasets/master/json/philadelphia-solar-installations.json").done(function(solarData){
  console.log(JSON.parse(solarData));
  _.each(JSON.parse(solarData), function(num){
    L.marker([num.Y, num.X]).addTo(map);
  });
});
